...
namespace ImageProcessingDemo1
{
    class ImageEffectManager:Control
    {
        ...
        public Image MakeWaterMark(Image sourceImage)
        {
        ...
        public Image resizeImage(Image sourceImage, int targetWidth, int targetHeight)
        {
            int width;//图片最终的宽
            int height;//图片最终的高
            try
        ...
        public Image DrawOutCropArea(Image image, int xPosition, int yPosition, int width, int height)

        {
            Bitmap _bitmapPrevCropArea = oldBitmap.Clone() as Bitmap;
        ...
        public Image Crop(int xPosition, int yPosition, int width, int height)
        {
            Bitmap bmap = oldBitmap.Clone() as Bitmap;

            if (xPosition + width > oldBitmap.Width)

                width = oldBitmap.Width - xPosition;

            if (yPosition + height > oldBitmap.Height)

                height = oldBitmap.Height - yPosition;
        ...
        public Image InsertImage(Image insertImage, int xPosition, int yPosition)
        {
            //   Bitmap temp = oldBitmap;
            Bitmap bmap = (Bitmap)oldBitmap.Clone();
            Graphics gr = Graphics.FromImage(bmap);
